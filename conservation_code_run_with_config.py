...
"""
Version 22/08/22
Before running the code have the following files in the directory: 
- script file itself (conservation_code_run_with_config.py)
- MUSCLE executable,
- proteomes.fasta
- targets.fasta
- sites.csv
- Mapped_Uniprot_Species_Names.tsv
- configurations.ini
- link_to_config.py
...
###specify config settings in configurations.ini file (target species using uniprot species ID (see "Mapped_Uniprot_Species_Names.tsv"; target amino acid, substitution amino acid, Eval threshold for BLAST))
import link_to_config
config = link_to_config.read_config()
species_of_targets = config['Conservation_Code_Parameters']['species_of_targets']
target_amino_acid = config['Conservation_Code_Parameters']['target_amino_acid']
sub = config['Conservation_Code_Parameters']['sub']
eval_thres = float(config['Conservation_Code_Parameters']['eval_thres'])
...
for sequence in seq_outputs: 
    ...
    all_lengths=[] #list of all sequence lengths for orthologues per target
    for rec in SeqIO.parse(sequence, "fasta"): #extract the lengths of all orthologues per target
...
shutil.move('Top_BLAST_hits_per_target','ALL_OUTPUTS')
shutil.move('FASTA_sequences_of_top_hits_per_target','ALL_OUTPUTS')
shutil.move('Alignments_per_target','ALL_OUTPUTS')
shutil.move('aligned_orthologues_per_target.csv','ALL_OUTPUTS')
shutil.move('alltophits.csv','ALL_OUTPUTS')
shutil.move('blast_result_file.tsv','ALL_OUTPUTS')
...
shutil.move('blast_database.phr','ALL_OUTPUTS')
shutil.move('blast_database.pin','ALL_OUTPUTS')
shutil.move('blast_database.psq','ALL_OUTPUTS')
